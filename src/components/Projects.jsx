...
import { TiltCard, GradientText, ShinyText, AnimatedList } from './reactbits';
...
function ProjectCard({ project }) {
    return (
        <TiltCard
            ...
            className="h-full"
        >
            <div className="h-full rounded-2xl bg-white dark:bg-surface-dark border border-slate-200 dark:border-slate-700 overflow-hidden transition-all hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-none hover:border-primary/30">
                {/* Header with Image */}
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {project.isFeatured && (
                                <span className="px-2 py-0.5 bg-yellow-500/30 backdrop-blur text-yellow-300 text-xs font-bold rounded-full border border-yellow-500/30 flex items-center gap-1">
                                    ...
                                    <ShinyText text="Featured" speed={2} className="text-xs font-bold text-yellow-300" />
                                </span>
                            )}
                        </div>
                    ...
                    {project.topics && project.topics.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-4">
                            {project.topics.map((topic) => (
                                <span
                                    key={topic}
                                    className="px-2 py-0.5 bg-primary/10 text-primary text-xs font-medium rounded-full hover:bg-primary/20 transition-colors"
                                >
                                    {topic}
                                </span>
                            ))}
